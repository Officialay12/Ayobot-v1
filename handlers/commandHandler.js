...
class RateLimiter {
  ...
  isAllowed(id, commandName = null) {
    const now = Date.now();
    const key = commandName ? `${id}:${commandName}` : id;
...
async function loadAllModules() {
  ...
  const loaded = Object.values(MODULES).filter((m) =>
    Object.keys(m).some((k) => k !== "__raw" && typeof m[k] === "function"),
  ).length;

  cmdLog.div();
  cmdLog.success(
    `✅ Loaded ${loaded}/${Object.keys(moduleMap).length} modules`,
  );
  console.log();
}
...
export function registerAllCommands() {
  ...
  rb("menu", "menu", {
    category: "core",
    aliases: ["help", "commands", "h", "cmd", "cmds"],
  });
  rb("ping", "ping", {
    category: "core",
    aliases: ["pong", "latency", "ms", "alive"],
  });
  rb("status", "status", {
    category: "core",
    aliases: ["me", "whoami", "myinfo"],
  });
  rb("creator", "creator", {
    category: "core",
    aliases: ["dev", "owner", "author", "ayo"],
  });
  rb("github", "creatorGit", {
    category: "core",
    aliases: ["git", "repo", "source"],
  });
  rb("auto", "auto", {
    category: "core",
    aliases: ["autoreply", "toggleauto"],
  });
  rb("connect", "connectInfo", {
    category: "core",
    aliases: ["community", "links"],
  });
  rb("prefix", "prefixinfo", {
    category: "core",
    aliases: ["preinfo", "getprefix"],
  });
  rb("test", "test", {
    category: "debug",
    aliases: ["hello", "hi", "testbot"],
  });
  rb("start", "start", {
    category: "core",
    aliases: ["open", "init", "begin", "connectdm", "dmopen"],
  });
  rb("time", "time", {
    category: "info",
    aliases: ["worldtime", "timezone", "clock"],
  });
  rb("weather", "weather", {
    category: "info",
  ...
  rb("myip", "myip", {
    category: "web",
    aliases: ["myipaddr", "publicip", "whatismyip"],
  });
  rb("whois", "whois", { category: "web", aliases: ["domain", "domaininfo"] });
  rb("dns", "dns", {
    category: "web",
    aliases: ["dnslookup", "nslookup", "dig"],
  });
  rb("url", "url", { category: "web", aliases: ["urlinfo", "urlcheck"] });
  rb("fetch", "fetch", {
    category: "web",
    aliases: ["geturl", "curl", "httpget"],
  });
  rb("scrape", "scrape", {
    category: "web",
    aliases: ["scraper", "webscrape"],
  });
  rb("screenshot", "screenshot", {
    category: "web",
    aliases: ["ss", "capture", "snap"],
  });
  rb("inspect", "inspect", {
    category: "web",
    aliases: ["pageinspect", "analyze"],
  });
  rb("shorten", "shorten", {
    category: "web",
  ...
  rb("getpp", "getpp", {
    category: "profile",
    aliases: ["pp", "profilepic", "pfp"],
  });
  rb("getgpp", "getgpp", {
    category: "profile",
    groupOnly: true,
    aliases: ["gpp", "grouppic"],
  });
  ...
  rb("activate", "activate", {
    category: "group",
    groupOnly: true,
    adminOnly: true,
  });
  rb("deactivate", "deactivate", {
    category: "group",
    groupOnly: true,
    adminOnly: true,
  });
  rb("antilink", "antilink", {
    category: "group",
    groupOnly: true,
  ...
  // ── GAMES.JS ──────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("rps", fn(m, "rps", "rockPaperScissors"), {
        category: "games",
      ...
      count++;
    if (
      safeRegister("dice", fn(m, "dice", "rollDice"), {
        category: "games",
      ...
      count++;
    if (
      safeRegister("flip", fn(m, "coinFlip", "flip"), {
        category: "games",
      ...
      count++;
    if (
      safeRegister("trivia", fn(m, "trivia"), {
        category: "games",
        aliases: ["quiz"],
      })
    )
  ...
  // ── JOKES.JS ──────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("joke", fn(m, "joke", "getJoke", "randomJoke"), {
        category: "fun",
        aliases: ["laugh", "funny"],
      })
    )
      count++;
    if (
      safeRegister("roast", fn(m, "roast", "burnUser"), {
        category: "fun",
        aliases: ["burn", "insult"],
      })
    )
      count++;
    if (
      safeRegister("pickup", fn(m, "pickupLine", "pickupline", "pickup"), {
        category: "fun",
  ...
  // ── QUOTES.JS ─────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister(
        "quote",
        fn(m, "quote", "getQuote", "randomQuote", "motivation"),
        { category: "fun", aliases: ["motivation", "inspire"] },
      )
    )
      count++;
  }
  ...
  // ── CALCULATOR.JS ─────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("calc", fn(m, "calculate", "calc", "calculator"), {
        category: "tools",
  ...
  // ── DICTIONARY.JS ─────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("dict", fn(m, "dict", "define", "dictionary"), {
        category: "info",
        aliases: ["define", "meaning", "word"],
      })
    )
      count++;
  }
  ...
  // ── DOWNLOADER.JS (features folder) ───────────────────────────────────────
  {
    ...
    if (!m || Object.keys(m).filter((k) => k !== "__raw").length === 0) {
      ...
      cmdLog.debug(
        `Downloader exports: ${Object.keys(m)
          .filter((k) => k !== "__raw")
          .join(", ")}`,
      );
      ...
      if (
        safeRegister(
          "youtube",
          fn(m, "youtube", "yt", "ytdl", "downloadYouTube"),
          { category: "dl", aliases: ["yt", "ytdl", "ytinfo"] },
        )
      )
        count++;
      if (
        safeRegister("tiktok", fn(m, "tiktok", "tt", "downloadTikTok"), {
          category: "dl",
          aliases: ["tt", "tok", "tiktokdl"],
        })
      )
        count++;
      if (
        safeRegister("spotify", fn(m, "spotify", "sp", "downloadSpotify"), {
          category: "dl",
      ...
      if (
        safeRegister(
          "instagram",
          fn(m, "instagram", "ig", "downloadInstagram"),
          { category: "dl", aliases: ["ig", "insta", "igdl"] },
        )
      )
        count++;
      if (
        safeRegister("facebook", fn(m, "facebook", "fb", "downloadFacebook"), {
          category: "dl",
          aliases: ["fb", "fbdl"],
        })
      )
        count++;
      if (
        safeRegister("twitter", fn(m, "twitter", "x", "downloadTwitter"), {
          category: "dl",
          aliases: ["x", "tweet", "xdl"],
        })
      )
      ...
      if (
        safeRegister(
          "pinterest",
          fn(m, "pinterest", "pin", "searchPinterest"),
          { category: "dl", aliases: ["pin", "pinsearch"] },
        )
      )
        count++;
      if (
        safeRegister("dl", fn(m, "download", "dl", "universalDownload"), {
          category: "dl",
  ...
  // ── MUSIC.JS (fallback if downloader doesn't have play) ────────────────────
  {
    ...
    if (!primaryCommands.has("play")) {
      if (
        safeRegister("play", fn(m, "musicDownload", "play", "playMusic"), {
          category: "music",
    ...
    if (
      safeRegister("lyrics", fn(m, "musicLyrics", "lyrics", "getLyrics"), {
        category: "music",
      ...
      count++;
    if (
      safeRegister("trending", fn(m, "musicTrending", "trending", "charts"), {
        category: "music",
        aliases: ["chart", "topsongs"],
      })
    )
      count++;
    if (
      safeRegister("musicsearch", fn(m, "musicSearch", "search", "findSong"), {
        category: "music",
        aliases: ["songsearch", "findmusic"],
      })
    )
      count++;
  }
  ...
  // ── IMAGE TOOLS.JS ────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister(
        "sticker",
        fn(m, "sticker", "makeSticker", "createSticker"),
        { category: "media", aliases: ["s", "stk", "makesticker"] },
      )
    )
      count++;
    if (
      safeRegister("toimage", fn(m, "toImage", "stickerToImage"), {
        category: "media",
        aliases: ["toimg"],
      })
    )
      count++;
    if (
      safeRegister("tovideo", fn(m, "toVideo", "stickerToVideo"), {
        category: "media",
        aliases: ["tovid"],
      })
    )
      count++;
    if (
      safeRegister("togif", fn(m, "toGif", "makeGif"), {
        category: "media",
        aliases: ["makegif"],
      })
    )
      count++;
    if (
      safeRegister("toaudio", fn(m, "toAudio", "extractAudio"), {
        category: "media",
      ...
      count++;
    if (
      safeRegister("removebg", fn(m, "removeBg", "removeBG", "rmbg"), {
        category: "media",
        aliases: ["nobg", "rmbg"],
      })
    )
      count++;
    if (
      safeRegister("meme", fn(m, "meme", "makeMeme"), {
        category: "media",
        aliases: ["makememe"],
      })
    )
  ...
  // ── AI.JS ─────────────────────────────────────────────────────────────────
  {
    const m = MODULES.ai;
    if (
      safeRegister("ayobot", fn(m, "ai", "chat", "ayobot"), {
        category: "ai",
        aliases: ["chat", "bae", "aichat", "askai"],
      })
    )
      count++;
    if (
      safeRegister("aiclear", fn(m, "aiClear", "clearChat"), {
        category: "ai",
      ...
      count++;
    if (
      safeRegister("summarize", fn(m, "summarize", "summary"), {
        category: "ai",
      ...
      count++;
    if (
      safeRegister("grammar", fn(m, "grammar", "spellcheck"), {
        category: "ai",
        aliases: ["spellcheck", "proofread"],
      })
    )
      count++;
  }
  ...
  // ── ADMIN.JS ──────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("mode", fn(m, "mode", "setMode", "botMode"), {
        category: "admin",
        adminOnly: true,
      ...
      count++;
    if (
      safeRegister("adduser", fn(m, "addUser", "adduser", "authorize"), {
        category: "admin",
        adminOnly: true,
    ...
    if (
      safeRegister(
        "removeuser",
        fn(m, "removeUser", "removeuser", "deauthorize"),
        { category: "admin", adminOnly: true, aliases: ["deauth"] },
      )
    )
      count++;
    if (
      safeRegister("listusers", fn(m, "listUsers", "listusers", "users"), {
        category: "admin",
        adminOnly: true,
        aliases: ["users", "authlist"],
      })
    )
      count++;
    if (
      safeRegister("broadcast", fn(m, "broadcast", "bc"), {
        category: "admin",
        adminOnly: true,
        aliases: ["bc", "announce"],
      })
    ...
    if (
      safeRegister(
        "globalbc",
        fn(m, "globalBroadcast", "globalbc", "globalBc"),
        { category: "admin", adminOnly: true, aliases: ["gbc"] },
      )
    )
      count++;
    if (
      safeRegister("stats", fn(m, "stats", "botStats", "botstats"), {
        category: "admin",
        adminOnly: true,
      ...
      count++;
    if (
      safeRegister("botstatus", fn(m, "botStatus", "botstatus", "fullStatus"), {
        category: "admin",
        adminOnly: true,
        aliases: ["botinfo", "fullstatus"],
      })
    )
      count++;
    if (
      safeRegister("superban", fn(m, "superBan", "superban", "globalBan"), {
        category: "admin",
        adminOnly: true,
        aliases: ["globalban", "permban"],
      })
    )
      count++;
    if (
      safeRegister("superunban", fn(m, "unban", "superUnBan", "globalUnban"), {
        category: "admin",
        adminOnly: true,
        aliases: ["globalunban"],
      })
      ...
      count++;
    if (
      safeRegister("listglobalbanned", fn(m, "listBanned", "listbanned"), {
        category: "admin",
        adminOnly: true,
        aliases: ["globalbannedlist"],
      })
    )
      count++;
    if (
      safeRegister("clearbans", fn(m, "clearBans", "clearbans"), {
        category: "admin",
        adminOnly: true,
        aliases: ["resetbans"],
      })
      ...
      count++;
    if (
      safeRegister("restart", fn(m, "restart", "reboot"), {
        category: "admin",
        adminOnly: true,
        aliases: ["reboot"],
      })
      ...
      count++;
    if (
      safeRegister("shutdown", fn(m, "shutdown", "stop"), {
        category: "admin",
        adminOnly: true,
        aliases: ["stop", "botoff"],
      })
      ...
      count++;
    if (
      safeRegister("eval", fn(m, "adminEval", "eval", "exec"), {
        category: "admin",
        adminOnly: true,
        aliases: ["exec", "code", "run"],
      })
    )
      count++;
  }
  ...
  // ── GROUP CORE.JS ─────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("kick", fn(m, "kick", "remove"), {
        category: "group",
        groupOnly: true,
      ...
      count++;
    if (
      safeRegister("add", fn(m, "add", "invite"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        requireBotAdmin: true,
      ...
      count++;
    if (
      safeRegister("promote", fn(m, "promote", "makeAdmin"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        requireBotAdmin: true,
      ...
      count++;
    if (
      safeRegister("demote", fn(m, "demote", "unadmin"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        requireBotAdmin: true,
      ...
      count++;
    if (
      safeRegister("admins", fn(m, "admins", "listAdmins"), {
        category: "group",
        groupOnly: true,
      ...
      count++;
    if (
      safeRegister("tagall", fn(m, "tagall", "everyone"), {
        category: "group",
        groupOnly: true,
      ...
      count++;
    if (
      safeRegister("hidetag", fn(m, "hidetag", "htag"), {
        category: "group",
        groupOnly: true,
        aliases: ["htag", "silent"],
      })
      ...
      count++;
    if (
      safeRegister("testadmin", fn(m, "testAdmin", "testadmin"), {
        category: "group",
        groupOnly: true,
      ...
      count++;
    if (
      safeRegister("refreshadmin", fn(m, "refreshAdmin", "refreshadmin"), {
        category: "group",
        groupOnly: true,
  ...
  // ── GROUP MODERATION.JS ───────────────────────────────────────────────────
  {
    ...
    if (
      ...
      count++;
    if (
      safeRegister("warnings", fn(m, "warnings"), {
        category: "group",
        groupOnly: true,
        aliases: ["warnlist", "mywarnings"],
      })
    )
      count++;
    if (
      safeRegister("clearwarns", fn(m, "clearWarns", "clearwarns"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["resetwarns"],
    ...
    if (
      ...
      count++;
    if (
      safeRegister("listbanned", fn(m, "listBanned", "listbanned"), {
        category: "group",
        groupOnly: true,
        aliases: ["bannedlist"],
      })
  ...
  // ── GROUP SETTINGS.JS ─────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("mute", fn(m, "mute"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["lockgroup", "muteall"],
      })
    )
      count++;
    if (
      safeRegister("unmute", fn(m, "unmute"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["unlockgroup"],
    ...
    if (
      ...
      count++;
    if (
      safeRegister("unlock", fn(m, "unlock"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["unlockinfo"],
      ...
      count++;
    if (
      safeRegister("antispam", fn(m, "antiSpam", "antispam"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["nospam"],
      ...
      count++;
    if (
      safeRegister("welcome", fn(m, "welcomeToggle", "welcome"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
      ...
      count++;
    if (
      safeRegister("setwelcome", fn(m, "setWelcome", "setwelcome"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
      ...
      count++;
    if (
      safeRegister("goodbye", fn(m, "goodbyeToggle", "goodbye"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
      ...
      count++;
    if (
      safeRegister("setgoodbye", fn(m, "setGoodbye", "setgoodbye"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
      ...
      count++;
    if (
      safeRegister("groupinfo", fn(m, "groupInfo", "groupinfo"), {
        category: "group",
        groupOnly: true,
        aliases: ["ginfo", "grouppanel"],
      })
    )
      count++;
    if (
      safeRegister("rules", fn(m, "rules"), {
        category: "group",
        groupOnly: true,
      ...
      count++;
    if (
      safeRegister("setrules", fn(m, "setRules", "setrules"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["setgrules"],
    ...
    if (
      ...
      count++;
    if (
      safeRegister("revoke", fn(m, "revoke"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
    ...
    if (
      ...
      count++;
    if (
      safeRegister("delete", fn(m, "deleteMsg", "delete"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["delmsg"],
      ...
      count++;
    if (
      safeRegister("settings", fn(m, "settingsOverview", "settings"), {
        category: "group",
        groupOnly: true,
      ...
      count++;
    if (
      safeRegister("resetsettings", fn(m, "resetSettings", "resetsettings"), {
        category: "group",
        groupOnly: true,
        requireGroupAdmin: true,
        aliases: ["resetgroupsettings"],
      })
    )
      count++;
    if (
      safeRegister("leave", fn(m, "leave"), {
        category: "group",
        groupOnly: true,
        adminOnly: true,
        aliases: ["botleave", "exit"],
      })
    )
      count++;
    if (
      safeRegister("groupdebug", fn(m, "debug"), {
        category: "group",
        groupOnly: true,
        aliases: ["gdebug"],
      })
      ...
      count++;
    if (
      safeRegister("participants", fn(m, "showParticipants", "participants"), {
        category: "group",
        groupOnly: true,
  ...
  // ── ENCRYPTION.JS ─────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("encrypt", fn(m, "encrypt"), {
        category: "security",
        aliases: ["enc"],
      })
    )
      count++;
    if (
      safeRegister("decrypt", fn(m, "decrypt"), {
        category: "security",
        aliases: ["dec"],
      })
    )
    ...
    if (
      ...
      count++;
    if (
      safeRegister("password", fn(m, "password", "genPass", "passwordGen"), {
        category: "security",
  ...
  // ── NOTES.JS ──────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("note", fn(m, "note", "saveNote"), {
        category: "storage",
        aliases: ["savenote", "remember"],
      })
    )
      count++;
    if (
      safeRegister("getnote", fn(m, "getnote", "getNote", "recall"), {
        category: "storage",
        aliases: ["recall", "readnote"],
      })
    )
      count++;
    if (
      safeRegister("notes", fn(m, "notes", "listNotes", "mynotes"), {
        category: "storage",
        aliases: ["mynotes", "listnotes"],
      })
    )
      count++;
    if (
      safeRegister(
        "delnote",
        fn(m, "deleteKey", "delnote", "deleteNote", "forget"),
        { category: "storage", aliases: ["forget", "deletenote"] },
      )
    )
      count++;
  }
  ...
  // ── REMINDER.JS ───────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("remind", fn(m, "reminder", "remind", "setReminder"), {
        category: "storage",
      ...
      count++;
    if (
      safeRegister("reminders", fn(m, "listReminders", "reminders"), {
        category: "storage",
        aliases: ["myreminders"],
      })
    )
    ...
    if (
      safeRegister(
        "cancelreminder",
        fn(m, "cancelReminder", "cancelreminder"),
        { category: "storage", aliases: ["delreminder"] },
      )
    )
      count++;
    if (
      safeRegister("snooze", fn(m, "snooze"), {
        category: "storage",
  ...
  // ── TRANSLATION.JS ────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("translate", fn(m, "translate"), {
        category: "tools",
        aliases: ["tr", "tl"],
      })
    )
      count++;
    if (
      safeRegister("detect", fn(m, "detect"), {
        category: "tools",
        aliases: ["langdetect"],
      })
    )
      count++;
    if (
      safeRegister("languages", fn(m, "languages"), {
        category: "tools",
        aliases: ["langs"],
      })
    )
  ...
  // ── UNIT CONVERTER.JS ─────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("convert", fn(m, "convert"), {
        category: "tools",
        aliases: ["conv", "cvt"],
      })
    )
  ...
  // ── SECURITY.JS ───────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("scan", fn(m, "scan", "urlScan", "scanUrl"), {
        category: "security",
        aliases: ["urlscan", "checksafe"],
      })
    )
      count++;
  }
  ...
  // ── STOCKS.JS ─────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("stock", fn(m, "stock", "stocks"), {
        category: "info",
  ...
  // ── NEWS.JS ───────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("news", fn(m, "news", "getNews"), {
        category: "info",
  ...
  // ── MOVIES.JS ─────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("movie", fn(m, "movie", "movies"), {
        category: "info",
        aliases: ["movies", "film"],
      })
    )
      count++;
    if (
      safeRegister("series", fn(m, "series", "tvshow"), {
        category: "info",
        aliases: ["tvshow"],
      })
    )
  ...
  // ── CRYPTO.JS ─────────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("crypto", fn(m, "crypto", "getCrypto", "coin"), {
        category: "info",
        aliases: ["coin", "cryptoprice"],
      })
    )
      count++;
  }
  ...
  // ── AUTOMATION.JS ─────────────────────────────────────────────────────────
  {
    ...
    if (
      safeRegister("autoreply", fn(m, "autoReply", "autoreply"), {
        category: "automation",
      })
    )
      count++;
    if (
      safeRegister("autosticker", fn(m, "autoSticker", "autosticker"), {
        category: "automation",
      })
    )
      count++;
...
async function resolveEffectiveBotAdmin(sock, groupJid, ownerPhone) {
  const ownerJid = ownerPhone
    ? `${normalizePhone(ownerPhone)}@s.whatsapp.net`
    : null;
...
export async function handleCommand(message, sock) {
  ...
  try {
    ...
    cmdLog.cmd(
      `[${executionId}] ${ENV.PREFIX}${commandName} → ${primaryName} | ${cleanPhone}${isGroup ? " [G]" : ""}`,
    );
    ...
    try {
      ...
      commandStats.set(primaryName, stats);
      metrics.errors++;
      if (metrics.errors > 10) cmdLog.warn("High error rate detected");
...
async function backupToFile(data, filename) {
  try {
    ...
    cmdLog.err(`Backup failed: ${error.message}`);
  }
}
...
export { commandCooldown, MODULES as modules, rateLimiter, metrics };
