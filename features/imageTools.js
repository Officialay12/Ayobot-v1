...
// ════════════════════════════════════════════════════════════════════════════
//  Image Tools Module — WITH PROPER STICKER METADATA EMBEDDING
//  Author: AYOCODES
//
//  FIXES:
//    • Sticker pack name appears as "AYOBOT V1" in sticker info
//    • Creator appears as "AYOCODES"
//    • Proper EXIF metadata embedded in sticker file
//    • No separate text messages after stickers
// ════════════════════════════════════════════════════════════════════════════
...
//
//  This makes the sticker pack name appear when users view sticker info:
//  • Pack name: "AYOBOT V1" (appears under the sticker like "Sticker.ly * AYOBOT V1")
//  • Publisher: "AYOCODES" (appears as the creator)
//
//  When users tap "View sticker pack" or check sticker info, they will see:
//  ┌─────────────────────────────┐
//  │  ⚡ AYOBOT V1               │
//  │     by AYOCODES            │
//  └─────────────────────────────┘
...
function buildStickerExif(packName = "AYOBOT V1", publisher = "AYOCODES") {
  ...
  // EXIF header required for WebP stickers (WhatsApp format)
  // This is the correct format that makes pack name appear in sticker info
...
// Function to embed EXIF into WebP sticker
async function embedStickerExif(webpBuffer, exifBuffer) {
  // For WebP stickers, we need to append the EXIF data properly
  // WhatsApp expects EXIF data appended to the WebP file
  return Buffer.concat([webpBuffer, exifBuffer]);
}

// ════════════════════════════════════════════════════════════════════════════
//  STICKER — Creates sticker with AYOBOT V1 branding EMBEDDED
//
//  When users view sticker info, they will see:
//  • Pack name: "AYOBOT V1" (embedded in the sticker file)
//  • Publisher: "AYOCODES"
//  • Emojis: 🤖 ⚡ 👑 🎭
//
//  NO separate text message is sent after the sticker
// ════════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (!quoted || (!quoted.imageMessage && !quoted.videoMessage)) {
      return sock.sendMessage(from, {
        text: formatInfo(
          "🎭 STICKER",
          `Reply to an image or video with .sticker\n\n✨ *AYOBOT V1* will appear as the sticker pack name when viewed!`,
        ),
      });
    }
    ...
    if (!isVideo) {
      ...
      // Embed the EXIF metadata into the sticker
      const finalSticker = await embedStickerExif(stickerBuf, exif);

      // Send ONLY the sticker — no separate text message
...
export async function toImage({ message, from, sock }) {
  try {
    ...
    if (!quoted || !quoted.stickerMessage) {
      return sock.sendMessage(from, {
        text: formatInfo("🖼️ STICKER → IMAGE", `Reply to a sticker with .toimage`),
      });
    }
...
export async function toVideo({ message, from, sock }) {
  try {
    ...
    if (!quoted || !quoted.stickerMessage) {
      return sock.sendMessage(from, {
        text: formatInfo("🎬 STICKER → VIDEO", `Reply to an animated sticker with .tovideo`),
      });
    }
    ...
    if (!hasFfmpeg) {
      return sock.sendMessage(from, {
        text: formatError("FFMPEG MISSING", "ffmpeg is not installed on this server."),
      });
    }
...
export async function removeBg({ message, from, sock }) {
  try {
    ...
    if (!quoted || !quoted.imageMessage) {
      return sock.sendMessage(from, {
        text: formatInfo("✨ REMOVE BACKGROUND", `Reply to an image with .removebg`),
      });
    }
    ...
    if (resultBuffer) {
      ...
      await sock.sendMessage(from, {
        text: formatError("REMOVEBG FAILED", "Could not remove background. Set REMOVEBG_KEY for better results."),
      });
    }
  } catch (e) {
...
export async function meme({ message, fullArgs, from, sock }) {
  try {
    ...
    if (!quoted || !quoted.imageMessage) {
      return sock.sendMessage(from, {
        text: formatInfo("🎭 MEME GENERATOR", `Reply to an image with:\n.meme <top text> | <bottom text>`),
      });
    }

    if (!fullArgs || !fullArgs.includes("|")) {
      return sock.sendMessage(from, {
        text: formatError("MEME ERROR", `Missing text separator |\n\nFormat: .meme Top | Bottom`),
      });
    }
