...
// ════════════════════════════════════════════════════════════════════════════
//  Group Settings Module - CLEAN VERSION
//  Author: AYOCODES
//
...
import { groupSettings, isAdmin, saveGroupSettings, groupWarnings } from '../../index.js';
import {
  clearGroupCache,
...
import { formatData, formatError, formatInfo, formatSuccess } from '../../utils/formatters.js';
...
// ============================================================================
//  UNLOCK GROUP (allow all members to edit info)
// ============================================================================
...
export async function rules({ from, sock }) {
  try {
    ...
    const rules = (groupSettings.get(from) || {}).rules || 'No rules set.\n\nAdmins: .setrules <rules>';
...
export async function link({ from, userJid, sock }) {
  try {
    ...
    if (await isBotGroupAdminCached(from, sock)) {
      try {
...
export async function tagAll({ args, fullArgs, message, from, userJid, sock }) {
  try {
    ...
    const quotedMsg = getReplyContext(message)?.stanzaId && getReplyContext(message)?.quotedMessage ? {
      key: {
        remoteJid: from,
        fromMe: phone(getReplyContext(message).participant) === phone(sock.user?.id),
...
export async function pin({ message, from, userJid, sock }) {
  try {
    ...
    await sock.sendMessage(from, { pin: { key, type: 1, time: 7 * 24 * 60 * 60 } }); // 7 days
...
export async function debug({ from, userJid, sock }) {
  try {
    ...
    const metadata = await getGroupMetadataCached(from, sock);
    const botNumber = phone(sock.user?.id || '');
